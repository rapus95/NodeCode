...
public class Grid {
	...
	public static boolean connect(boolean prog, Node source, int indexOut, Node target, int indexIn){
		if(prog)
			return connect(source.getProgOut(indexOut), target.getProgIn());
	...
	public static boolean connect(PinOutput out, PinInput in){
		...
		|| out instanceof ValueHandler && in instanceof ValueHandler && ((ValueHandler<?>) out).getType()==((ValueHandler<?>) in).getType()
		){
